...
vec2 map(vec3 p)
{
    ...
    p = rotx(p,time*1.0+rid+sin(rid*5.+time));
    p = roty(p,time*1.0+rid*1.1);
...
{	
    ...
    //camera
    vec3 ro = vec3(4.1,-time*4.,time-0.5);
    vec3 rd = normalize(vec3(p,2.5));
    rd = roty(rd,sin(time*0.4)*0.5+0.9+mo.x);
    rd = rotx(rd,sin(time*0.24+sin(time*1.0)*1.)+0.9);
