...
void* render_loop(void *arg) {
    ...
    for (;;) {
        ...
        scene->bcm_mapper(scene, dmxData);  // Render image buffer in scene
...
int main(int argc, char **argv) {
    ...
    // Initial pixel set
    int pixelIndex = (scene->width * (scene->height + 1)/2) * scene->stride;
    dmxData[pixelIndex] = 255;
    dmxData[pixelIndex + 1] = 255;
    dmxData[pixelIndex + 2] = 255;
